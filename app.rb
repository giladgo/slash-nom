...
def go(rest, params)
  ...
  if decl.new_record?
    decl.save!
    respond_in_channel "#{decl.user_name} wants to go to #{decl.restaurant.display_name}! Join them by entering `/um go #{decl.restaurant.name}`, or show an interest in a different place by typing `/um go [other-place]`."
  else
